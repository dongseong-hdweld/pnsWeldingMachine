...
function StepChip({ index, current, setCurrent, label, locked }) {
  ...
  return (
    ...
    >
      <span className="inline-flex items-center justify-center w-5 h-5 rounded-full border mr-1
...
function Modal({ title, children, onClose }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-[92vw] max-w-xl rounded-2xl border
...
export default function Register() {
  ...
  useEffect(() => { setModel(''); }, [category]); // 카테고리 바뀌면 2차 모델 초기화
  ...
  const resetAll = () => {
    setStep(0);
    setCategory(''); setModel(''); setSerial(''); setPurchaseDate(''); setVendor(''); setInvoiceFiles([]);
    setFirstName(''); setSurName(''); setPhoneCode('+82'); setPhoneLocal(''); setEmail(''); setZip(''); setAddress('');
    setEmailSent(false); setEmailCode(''); setEmailVerified(false); setEmailMsg(''); setEmailErr('');
    setConsentService(false); setConsentXBorder(false); setConsentMarketing(false);
  };
      ...
      <Card title="1. 제품정보">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Input id="serial" value={serial} onChange={(e) => setSerial(e.target.value)} placeholder="예: HYW-ABC123456" />
          </div>
          ...
          <div>
            ...
            <Input id="vendor" value={vendor} onChange={(e) => setVendor(e.target.value)} placeholder="예: 현대 PNS 대리점" />
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 file:mr-3 file:rounded-lg file:border file:px-3 file:py-2
        ...
        <div className="mt-4 flex gap-2">
          <Button
            onClick={() => setStep(2)}
            disabled={!validStep1}
            className={validStep1 ? '' : 'opacity-50 cursor-not-allowed'}
          >
            다음(고객정보)
          </Button>
        </div>
        ...
        <div className={step < 2 ? 'pointer-events-none opacity-60' : ''}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="md:col-span-2">
              ...
              {/* 인증 메시지 */}
              {emailMsg && (
                <p className="text-emerald-500 text-xs mt-1">{emailMsg}</p>
              )}
              {emailErr && (
                <p className="text-rose-500 text-xs mt-1">{emailErr}</p>
              )}
            ...
            <div className="md:col-span-2">
              ...
              <Textarea id="addr" rows={3} value={address} onChange={(e) => setAddress(e.target.value)} placeholder="도로명, 동/호수, 도시, 국가 등" />
            </div>
          </div>
          ...
          <div className="mt-4 flex gap-2">
            ...
            <Button
              onClick={() => setStep(3)}
              disabled={!validStep2}
              className={validStep2 ? '' : 'opacity-50 cursor-not-allowed'}
            >
              다음(개인정보 동의)
            </Button>
          </div>
        ...
        <div className={step < 3 ? 'pointer-events-none opacity-60' : ''}>
          <div className="space-y-3 text-sm">
            <label className="flex items-start gap-2">
              ...
              <span className="text-slate-700 dark:text-slate-200">
                ...
                (서비스 제공을 위한 고객 식별, 구매내역 확인, A/S 일정 안내 및 처리)
              </span>
            </label>
            ...
            <label className="flex items-start gap-2">
              ...
              <span className="text-slate-700 dark:text-slate-200">
                ...
                (해외 본사/서비스센터로의 데이터 전송 시 표준계약조항 등 적정 보호조치 적용)
              </span>
            </label>
            ...
            <label className="flex items-start gap-2">
              ...
              />
              <span className="text-slate-700 dark:text-slate-200">
                <strong className="font-semibold">[필수]</strong> 제품 업데이트/안전 공지 등 정보 수신에 동의합니다.
                (선택적 프로모션 포함 가능, 언제든지 수신거부 가능)
              </span>
            </label>
          ...
          <div className="mt-4 flex gap-2">
            ...
            <Button
              onClick={() => setStep(4)}
              disabled={!validStep3}
              className={validStep3 ? '' : 'opacity-50 cursor-not-allowed'}
            >
              다음(확인)
            </Button>
          </div>
