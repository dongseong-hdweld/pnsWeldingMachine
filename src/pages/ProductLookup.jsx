...
export default function ProductLookup() {
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter((d) => {
      ...
      const c = d.customer || {}
      return [
        p.productName, p.model, p.serial, p.vendor,
  ...
  const isSameAsCurrent = emailInput.trim().toLowerCase() === (email || '').trim().toLowerCase()
  ...
  return (
    <PageWrap title="제품 조회" subtitle="인증된 이메일로 등록한 제품을 확인">
      ...
      {/* 검색 바 */}
      <Card className="mb-4">
        <Label htmlFor="q">제품/고객 검색</Label>
        <Input
          id="q"
          placeholder="모델, 시리얼, 구매처, 고객명/이메일/전화 등"
          value={q}
      ...
      {filtered.length === 0 ? (
        <Card>
          ...
          <div className="mt-2 text-sm">
            <a className="underline" href="/register">→ 제품 등록 페이지로 이동</a>{' '}
            <span className="text-slate-500 dark:text-slate-400">또는</span>{' '}
            <a className="underline" href="/manuals">→ 메뉴얼 검색으로 이동</a>
          </div>
        </Card>
      ) : (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {filtered.map((d, idx) => {
            ...
            return (
              <Card key={idx} className="rounded-2xl border border-slate-200 shadow-sm dark:border-slate-700">
                <div className="flex items-center justify-between">
                  <h3 className="text-base font-semibold">{title}</h3>
                  <div className="text-xs text-slate-500 dark:text-slate-400">
                    등록일시: {d.createdAt?.replace('T',' ').replace('Z',' UTC') || '-'}
                  </div>
                </div>
                ...
                <div className="mt-3 grid grid-cols-1 gap-3">
                  ...
                  <section className="rounded-xl border border-slate-200 p-3 dark:border-slate-700">
                    <h4 className="font-medium flex items-center gap-2"><span>📦</span> 제품정보</h4>
                    <dl className="mt-2 grid grid-cols-3 gap-x-3 gap-y-1 text-sm">
                      <dt className="text-slate-500 dark:text-slate-400">모델(분류)</dt>
                  ...
                  {/* 고객정보 */}
                  <section className="rounded-xl border border-slate-200 p-3 dark:border-slate-700">
                    <h4 className="font-medium flex items-center gap-2"><span>🙍</span> 고객정보</h4>
                    <dl className="mt-2 grid grid-cols-3 gap-x-3 gap-y-1 text-sm">
                      <dt className="text-slate-500 dark:text-slate-400">이름</dt>
                      <dd className="col-span-2">{c.firstName || '-'} {c.surName || ''}</dd>
                  ...
                  {/* 개인정보 동의 */}
                  <section className="rounded-xl border border-slate-200 p-3 dark:border-slate-700">
                    <h4 className="font-medium flex items-center gap-2"><span>🔒</span> 개인정보 동의</h4>
                      ...
                      <li className="flex items-center justify-between rounded-xl border border-slate-200 px-3 py-2 dark:border-slate-700">
                        ...
                        <span className={[
                          ...
                          priv.consentService
                            ? 'border-emerald-300 bg-emerald-50 text-emerald-700'
                            : 'border-rose-300 bg-rose-50 text-rose-700'
                        ].join(' ')}>{priv.consentService ? '동의' : '미동의'}</span>
                      </li>
                      <li className="flex items-center justify-between rounded-xl border border-slate-200 px-3 py-2 dark:border-slate-700">
                        ...
                        <span className={[
                          ...
                          priv.consentXBorder
                            ? 'border-emerald-300 bg-emerald-50 text-emerald-700'
                            : 'border-rose-300 bg-rose-50 text-rose-700'
                        ].join(' ')}>{priv.consentXBorder ? '확인' : '미확인'}</span>
                      </li>
                      <li className="flex items-center justify-between rounded-xl border border-slate-200 px-3 py-2 dark:border-slate-700">
                        ...
                        <span className={[
                          ...
                          priv.consentMarketing
                            ? 'border-emerald-300 bg-emerald-50 text-emerald-700'
                            : 'border-rose-300 bg-rose-50 text-rose-700'
                        ].join(' ')}>{priv.consentMarketing ? '동의' : '미동의'}</span>
                      </li>
                    </ul>
                  </section>
                  ...
                  {/* 첨부 인보이스 */}
                  <section className="rounded-xl border border-slate-200 p-3 dark:border-slate-700">
                    <h4 className="font-medium flex items-center gap-2"><span>🧾</span> 첨부 인보이스</h4>
                    {Array.isArray(p.invoiceFiles) && p.invoiceFiles.length > 0 ? (
                      <ul className="mt-2 space-y-1 text-sm">
                        {p.invoiceFiles.map((f, i) => (
                          <li key={i} className="flex items-center justify-between rounded-lg border border-slate-200 px-3 py-2 dark:border-slate-700">
                            <div className="truncate">
                              <div className="font-medium truncate">{f.name || '-'}</div>
                              <div className="text-xs text-slate-500 dark:text-slate-400">{f.type || '-'} · {formatSize(f.size)}</div>
                            </div>
                            <span className="text-xs text-slate-400">미리보기/다운로드는 업로드 연동 후 제공</span>
                          </li>
                        ))}
                      </ul>
                    ) : (
                      <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">첨부된 인보이스가 없습니다.</p>
                    )}
                  </section>
                </div>
              </Card>
